...
if clair_bot:
    ...
    def clair_text(message):
        ...
        if admin and text == "💬 Main Menu":
            clair_start(message); return
        if text == "🔮 3-Question Text Reading":
            SESSIONS[message.from_user.id] = {"stage": "questions", "questions": [], "service": "3-Question Text Reading"}
            clair_bot.send_message(message.chat.id, "Lovely. Please send your first question."); return
        if text == "🎙️ 15-Minute Voice Note":
            SESSIONS[message.from_user.id] = {"stage": "voice", "service": "15-Minute Voice Note Reading"}
            clair_bot.send_message(message.chat.id, "Wonderful. Send one main question, related thoughts, or say ‘general reading’. You may also send a Telegram voice message."); return
        if text == "🌐 Renova Website":
            clair_bot.send_message(message.chat.id, WEBSITE_URL); return
        if text == "❓ Help":
            clair_bot.send_message(message.chat.id, "Choose a reading, answer Clair’s prompts, review your request, then tap Send to Jarrod."); return
        ...
        if session["stage"] == "questions":
            ...
            if n < 3: clair_bot.send_message(message.chat.id, f"Thank you — question {n} of 3 received.\n\nPlease send question {n + 1}.")
                ...
                session["stage"] = "review"; q = session["questions"]
                m = InlineKeyboardMarkup(row_width=1); m.add(InlineKeyboardButton("✅ Send to Jarrod", callback_data="submit"), InlineKeyboardButton("🔄 Start Again", callback_data="restart"))
                clair_bot.send_message(message.chat.id, f"🌙 Your questions are gathered.\n\n1. {q[0]}\n\n2. {q[1]}\n\n3. {q[2]}\n\nSend these through to Jarrod?", reply_markup=m)
        elif session["stage"] == "voice":
            session["context"] = text; session["stage"] = "review"
            m = InlineKeyboardMarkup(row_width=1); m.add(InlineKeyboardButton("✅ Send to Jarrod", callback_data="submit"), InlineKeyboardButton("🔄 Start Again", callback_data="restart"))
            clair_bot.send_message(message.chat.id, "🌙 I’ve gathered your request. Send it through to Jarrod?", reply_markup=m)

    @clair_bot.callback_query_handler(func=lambda call: True)
    def clair_callbacks(call):
        if call.data == "text":
            SESSIONS[call.from_user.id] = {"stage": "questions", "questions": [], "service": "3-Question Text Reading"}; clair_bot.answer_callback_query(call.id); clair_bot.send_message(call.message.chat.id, "Lovely. Please send your first question."); return
        if call.data == "voice":
            SESSIONS[call.from_user.id] = {"stage": "voice", "service": "15-Minute Voice Note Reading"}; clair_bot.answer_callback_query(call.id); clair_bot.send_message(call.message.chat.id, "Wonderful. Send one main question or say ‘general reading’."); return
        if call.data == "restart":
            SESSIONS.pop(call.from_user.id, None); clair_bot.answer_callback_query(call.id); clair_bot.send_message(call.message.chat.id, "The thread has been cleared. Choose a reading below.", reply_markup=customer_menu()); return
        if call.data == "submit":
            session = SESSIONS.get(call.from_user.id, {}); user = call.from_user
            ...
            if session.get("service") == "3-Question Text Reading":
                ...
                content = session.get("context", "General reading"); heading = "🎙️ NEW VOICE NOTE READING REQUEST"
            admin_message = f"{heading}\n\nClient: {name}\nUsername: {username}\nTelegram ID: {user.id}\nService: {session.get('service', 'Reading')}\n\n{content}"
            clair_bot.send_message(CLAIR_ADMIN_ID, admin_message)
            clair_bot.answer_callback_query(call.id); clair_bot.send_message(call.message.chat.id, "Thank you — your request has been sent through to Jarrod. He’ll return to you during standard hours.", reply_markup=customer_menu()); return

    @clair_bot.message_handler(content_types=["voice"])
    def clair_voice(message):
        session = SESSIONS.get(message.from_user.id)
        if session and session.get("stage") == "voice":
            session["voice_file_id"] = message.voice.file_id; session["stage"] = "review"
            m = InlineKeyboardMarkup(row_width=1); m.add(InlineKeyboardButton("✅ Send to Jarrod", callback_data="submit"), InlineKeyboardButton("🔄 Start Again", callback_data="restart"))
            clair_bot.send_message(message.chat.id, "🌙 I’ve received your voice note. Send it through to Jarrod?", reply_markup=m)
